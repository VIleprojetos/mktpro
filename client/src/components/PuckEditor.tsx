...
export function PuckEditor({ initialData, onSave, onBack }: PuckEditorProps) {
  ...
  useEffect(() => {
    ...
    if (initialData?.components && Array.isArray(initialData.components) && initialData.components.length > 0) {
        ...
        setHistory([{ components: JSON.parse(JSON.stringify(loadedComponents)), timestamp: Date.now() }]);
        setHistoryIndex(0);
        ...
        Array.from(tempDiv.children).forEach((element: Element) => {
            ...
            const puckType = htmlElement.getAttribute('data-puck-type') || '';
            const componentType = (puckType && COMPONENT_TYPES[puckType as keyof typeof COMPONENT_TYPES]) 
                ? puckType
                : 'text';

            parsedComponents.push({
                id: htmlElement.id || `comp_${Date.now()}`,
                type: componentType,
        ...
        setHistory([{ components: JSON.parse(JSON.stringify(parsedComponents)), timestamp: Date.now() }]);
        setHistoryIndex(0);
    } else {
        ...
        setHistory([{ components: JSON.parse(JSON.stringify(defaultComponents)), timestamp: Date.now() }]);
        setHistoryIndex(0);
    }
  ...
  const undo = () => { if (historyIndex > 0) { setHistoryIndex(prev => prev - 1); setComponents(history[historyIndex - 1].components); } };
  const redo = () => { if (historyIndex < history.length - 1) { setHistoryIndex(prev => prev + 1); setComponents(history[historyIndex + 1].components); } };
  
  const addComponent = (type: keyof typeof COMPONENT_TYPES) => {
    const componentType = COMPONENT_TYPES[type];
    const newComponent: EditorComponent = { id: `${type}_${Date.now()}`, type, content: componentType.defaultContent, styles: { ...componentType.defaultStyles } };
  ...
  const generateHtmlFromComponents = (isPreview: boolean) => {
    let html = '';
    components.forEach(component => {
        ...
        const attributes = `id="${component.id}" style="${styleString}" data-puck-type="${component.type}"`;
        const clickHandler = isPreview ? "alert('Button clicked!')" : "";
        
        if (component.type === 'image') {
            html += `<img ${attributes} src="${component.content}" alt="Image" class="hover-effect" />\n`;
        } else if (component.type === 'button') {
            html += `<button ${attributes} class="button-hover" ${isPreview ? `onclick="${clickHandler}"` : ''}>${component.content}</button>\n`;
        } else if (component.type === 'spacer') {
            html += `<div ${attributes}></div>\n`;
        } else {
            const tag = component.type === 'heading' ? 'h1' : 'div';
            const className = component.type === 'card' ? 'hover-effect' : '';
            html += `<${tag} ${attributes} class="${className}">${component.content}</${tag}>\n`;
        }
    });
    return html;
  };
  ...
  const handleSave = async () => {
    setIsSaving(true);
    const html = generateHtmlFromComponents(false);
    const css = `
    ...
    `;
    const data: Data = { html, css, components: components, styles: {} };
    await onSave(data);
    setIsSaving(false);
  };

  const handlePreview = () => {
    const html = generateHtmlFromComponents(true);
    const fullHtml = `
      <!DOCTYPE html>
      <html>
        <head>
    ...
    if (newWindow) { newWindow.document.write(fullHtml); newWindow.document.close(); }
  };

  const generateCode = () => generateHtmlFromComponents(false);

  const openContentEditor = (component: EditorComponent) => { setEditingContent(component.content); setShowContentEditor(true); };
  const saveContentEdit = () => { if (selectedComponent) { updateComponent(selectedComponent, { content: editingContent }); } setShowContentEditor(false); };

  const filteredComponents = Object.entries(COMPONENT_TYPES).filter(([type, config]) => activeCategory === 'all' || config.category === activeCategory);
  ...
  return (
    <div className="flex flex-col h-screen w-full bg-gray-950 text-gray-200">
       {/* O restante do JSX (Header, Sidebars, Canvas, Modals) permanece o mesmo. */}
      ...
      <div className="flex flex-1 overflow-hidden">
        <div className="w-64 bg-gray-900 border-r border-gray-700 flex flex-col">
          <div className="p-3 border-b border-gray-700">
            ...
            <div className="flex flex-wrap gap-1">
              ...
              {Object.entries(COMPONENT_CATEGORIES).map(([key, label]) => (<button key={key} className={`px-2 py-1 text-xs rounded ${activeCategory === key ? 'bg-blue-500 text-white' : 'bg-gray-700 text-gray-300 hover:bg-gray-600'}`} onClick={() => setActiveCategory(key)}>{label}</button>))}
            </div>
          </div>
          <div className="flex-1 overflow-auto p-2 space-y-1">
            {filteredComponents.map(([type, config]) => { const IconComponent = config.icon; return (<Button key={type} variant="ghost" size="sm" className="w-full justify-start text-gray-300 hover:bg-gray-700 hover:text-white" onClick={() => addComponent(type as keyof typeof COMPONENT_TYPES)}><IconComponent className="h-4 w-4 mr-2" />{config.label}</Button>); })}
          </div>
        </div>
        <div className="flex-1 flex flex-col bg-gray-950">
          <div className="flex-1 overflow-auto p-8" style={{ transform: `scale(${zoom / 100})`, transformOrigin: 'top left' }}>
            <div className="max-w-4xl mx-auto bg-gray-900 min-h-full shadow-lg rounded-lg overflow-hidden ring-1 ring-gray-800">
              {components.length === 0 ? (<div className="flex items-center justify-center h-64 text-gray-500"><div className="text-center"><Plus className="h-12 w-12 mx-auto mb-4 opacity-50" /><p>Adicione componentes para começar a construir sua página</p></div></div>) : (
                  ...
                  {components.map((component, index) => (
                    <div key={component.id} className={`relative group ${selectedComponent === component.id ? 'ring-2 ring-blue-500 ring-offset-2 ring-offset-gray-900' : ''}`} onClick={() => setSelectedComponent(component.id)}>
                      ...
                      <div style={component.styles}>
                        {component.type === 'image' ? (<img src={component.content} alt="Component" style={{ width: '100%', height: 'auto', display: 'block' }} />) : 
                         component.type === 'button' ? (<button style={{ all: 'inherit', display: 'inline-block' }}>{component.content}</button>) : 
                         component.type === 'spacer' ? (<div style={{ border: selectedComponent === component.id ? '1px dashed #3b82f6' : 'none', backgroundColor: selectedComponent === component.id ? 'rgba(59, 130, 246, 0.1)' : 'transparent' }} />) : 
        ...
        <div className="w-80 bg-gray-900 border-l border-gray-700 flex flex-col">
          ...
          <div className="flex-1 overflow-auto p-3">
            {selectedComponent ? (<div className="space-y-4">{(() => { const component = components.find(c => c.id === selectedComponent); if (!component) return null; return (
              <div className="space-y-6">
                <div className="p-3 bg-gray-800 rounded-lg"><label className="text-sm font-medium text-gray-300">Tipo de Componente</label><p className="text-sm text-blue-400 mt-1">{COMPONENT_TYPES[component.type as keyof typeof COMPONENT_TYPES]?.label}</p></div>
                <div className="space-y-3"><h4 className="font-medium text-sm text-gray-300 border-b border-gray-700 pb-2">Estilos</h4>
                  {Object.entries(component.styles).map(([key, value]) => (<div key={key}><label className="text-xs font-medium text-gray-400 capitalize">{key.replace(/([A-Z])/g, ' $1')}</label><input type="text" className="w-full mt-1 px-2 py-1 border rounded text-sm bg-gray-700 border-gray-600 text-gray-200" value={value as string} onChange={(e) => updateComponent(component.id, { styles: { ...component.styles, [key]: e.target.value } })} /></div>))}
                </div>
              </div>
            );})()}</div>) : (<div className="text-center py-12 text-gray-500"><Settings className="h-12 w-12 mx-auto mb-4 opacity-30" /><p className="text-sm">Selecione um componente para editar suas propriedades</p><p className="text-xs mt-1 opacity-75">Clique em qualquer elemento na tela para começar</p></div>)}
          </div>
        </div>
      </div>
      {showContentEditor && (<div className="fixed inset-0 bg-black bg-opacity-70 flex items-center justify-center z-50"><div className="bg-gray-800 rounded-lg p-6 w-[600px] max-w-full max-h-[80vh] flex flex-col border border-gray-700"><h3 className="text-lg font-semibold mb-4 text-gray-100">Editar Conteúdo</h3><div className="flex-1 min-h-0"><textarea className="w-full h-full p-3 border rounded-lg resize-none font-mono text-sm bg-gray-900 border-gray-600 text-gray-200 focus:ring-blue-500 focus:border-blue-500" value={editingContent} onChange={(e) => setEditingContent(e.target.value)} placeholder="Digite o conteúdo... (HTML é suportado)" /></div><div className="flex gap-2 mt-4"><Button onClick={saveContentEdit} className="bg-blue-600 hover:bg-blue-700 text-white"><Save className="h-4 w-4 mr-2" />Salvar</Button><Button variant="outline" onClick={() => setShowContentEditor(false)} className="border-gray-600 text-gray-300 hover:bg-gray-700 hover:text-white">Cancelar</Button></div></div></div>)}
      {showCode && (<div className="fixed inset-0 bg-black bg-opacity-70 flex items-center justify-center z-50"><div className="bg-gray-800 rounded-lg p-6 w-[800px] max-w-full max-h-[80vh] flex flex-col border border-gray-700"><div className="flex items-center justify-between mb-4"><h3 className="text-lg font-semibold text-gray-100">Código HTML Gerado</h3><Button variant="outline" size="sm" onClick={() => { navigator.clipboard.writeText(generateCode()); }} className="border-gray-600 text-gray-300 hover:bg-gray-700 hover:text-white"><Copy className="h-4 w-4 mr-2" />Copiar</Button></div><div className="flex-1 min-h-0"><pre className="bg-gray-900 p-4 rounded-lg text-sm overflow-auto h-full border border-gray-700"><code className="text-yellow-300">{generateCode()}</code></pre></div><div className="flex gap-2 mt-4"><Button variant="outline" onClick={() => setShowCode(false)} className="border-gray-600 text-gray-300 hover:bg-gray-700 hover:text-white">Fechar</Button></div></div></div>)}
    </div>
  );
}
...
export function usePageBuilder() {
  ...
  const loadData = (): Data | undefined => { return data || undefined; };
  return { data, isLoading, error, saveData, loadData };
}
