...
export function PuckEditor({ initialData, onSave, onBack }: PuckEditorProps) {
  ...
  // Função auxiliar para parsear elementos aninhados
  const parseElement = (element: HTMLElement): EditorComponent => {
  ...
  useEffect(() => {
    ...
    if (initialData?.components && Array.isArray(initialData.components) && initialData.components.length > 0) {
      ...
      Array.from(body.children).forEach((element) => {
        ...
        parsedComponents.push(parseElement(element));
      });
  ...
  const generateHtmlFromComponents = () => {
    ...
    const renderComponent = (id: string) => {
      ...
      if (component.children) {
        content += component.children.map(renderComponent).join('');
      }

      return `<div id="${component.id}" style="${styleString}">${content}</div>`;
    };

    components.forEach(component => {
      html += renderComponent(component.id);
    });
  ...
  return (
    <div className="flex flex-col h-screen w-full bg-gray-950 text-gray-200">
                ...
                return (
                  <div className="space-y-6">
                    <div className="p-3 bg-gray-800 rounded-lg">
                      ...
                      <p className="text-sm text-blue-400 mt-1">{COMPONENT_TYPES[component.type as keyof typeof COMPONENT_TYPES]?.label}</p>
                    </div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-70 flex items-center justify-center z-50">
          <div className="bg-gray-800 rounded-lg p-6 w-[600px] max-w-full max-h-[80vh] flex flex-col border border-gray-700">
            ...
            <div className="flex-1 min-h-0">
              <textarea className="w-full h-full p-3 border rounded-lg resize-none font-mono text-sm bg-gray-900 border-gray-600 text-gray-200" value={editingContent} onChange={(e) => setEditingContent(e.target.value)} />
            </div>
      ...
      {/* Modal para exibição do código HTML */}
      {showCode && (
        <div className="fixed inset-0 bg-black bg-opacity-70 flex items-center justify-center z-50">
          <div className="bg-gray-800 rounded-lg p-6 w-[800px] max-w-full max-h-[80vh] flex flex-col border border-gray-700">
            ...
            <div className="flex-1 min-h-0">
              <pre className="bg-gray-900 p-4 rounded-lg text-sm overflow-auto h-full border border-gray-700">
                <code className="text-yellow-300">{generateHtmlFromComponents()}</code>
              </pre>
            </div>
            <div className="flex gap-2 mt-4">
              <Button variant="outline" onClick={() => setShowCode(false)} className="border-gray-600 text-gray-300 hover:bg-gray-700 hover:text-white">Fechar</Button>
            </div>
          </div>
        </div>
      )}
