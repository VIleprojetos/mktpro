...
class GeminiService {
  ...
  constructor(apiKey: string) {
    if (!apiKey) {
      console.warn('[GeminiService] CONSTRUCTOR: API Key não configurada. O serviço não funcionará.');
      return;
    }
  ...
  public async createUltraLandingPage(
    ...
    try {
      console.log(`[GeminiService] LOG: Iniciando a geração da página para o prompt: "${prompt.substring(0, 50)}..."`);
      console.log(`[GeminiService] LOG: Opções:`, options);
      ...
      console.log(`[GeminiService] LOG: Geração da página concluída com sucesso.`);
      return htmlContent;
    } catch (error: any) {
      console.error('[GeminiService] ERRO FATAL ao gerar landing page ultra-premium. Verifique a chave da API e os limites de uso.', error);
  ...
  public async generatePremiumVariations(
    ...
    for (let i = 0; i < count; i++) {
      ...
      try {
        console.log(`[GeminiService] LOG: Iniciando a geração da Variação Premium ${i + 1}/${count}...`);
        const variation = await this.createUltraLandingPage(prompt, options);
        variations.push(variation);
        console.log(`[GeminiService] LOG: Variação Premium ${i + 1}/${count} gerada com sucesso.`);
        if (i < count - 1) {
          // Wait a bit to avoid hitting potential rate limits
          await new Promise(resolve => setTimeout(resolve, 1000));
        }
      } catch (error) {
        console.error(`[GeminiService] ERRO FATAL ao gerar a Variação Premium ${i + 1}.`, error);
