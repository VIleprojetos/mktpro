...
class GeminiService {
  ...
  public async createUltraLandingPage(
    ...
    const userPrompt = `
      ...
      ELEMENTOS A INCLUIR:
      ${options.includeTestimonials !== true ? '✅ Seção de Depoimentos com fotos e ratings' : ''}
      ${options.includePricing ? '✅ Seção de Preços com comparação de planos' : ''}
      ${options.includeStats !== true ? '✅ Estatísticas impressionantes com counter animations' : ''}
      ${options.includeFAQ !== true ? '✅ FAQ estratégico com accordion' : ''}
      ${options.includeVideo ? '✅ Seção de vídeo explicativo' : ''}
      ${options.includeNewsletter !== true ? '✅ Newsletter signup com incentivo' : ''}
      ${options.includeBlog ? '✅ Seção de blog/conteúdo' : ''}
      ${options.includeFeatures !== true ? '✅ Grid de recursos/funcionalidades' : ''}
  ...
  public async optimizeUltraLandingPage(
    ...
    const optimizationPrompt = `
      ...
      RESULTADO: Retorne TUDO otimizado completo, sem explicações.
